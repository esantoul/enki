...
TEST_CASE("C-Array Serialization and Deserialization")
{
  ...
  const auto ser_res = enki::BaseEngine::Serialize(carr1, temp.begin()).first;
  REQUIRE_NOTHROW(ser_res.or_throw());
  REQUIRE(ser_res.size() == sizeof(carr1));

  const auto des_res = enki::BaseEngine::Deserialize(carr2, temp.begin()).first;
...
TEST_CASE("C++ Array (std::array) Serialization and Deserialization")
{
  ...
  const auto ser_res = enki::BaseEngine::Serialize(arr1, temp.begin()).first;
  REQUIRE_NOTHROW(ser_res.or_throw());
  REQUIRE(ser_res.size() == sizeof(arr1));

  const auto des_res = enki::BaseEngine::Deserialize(arr2, temp.begin()).first;
  REQUIRE_NOTHROW(des_res.or_throw());
